...
<div class="md:hidden sticky bottom-0 w-full bg-white dark:bg-black z-[999999998]">
  <div
    class="relative w-full pb-[10px] flex items-center justify-between sm:justify-around gap-[15px] px-[35px] overflow-hidden"
  >
    <img src="/imgs/headerElipse.svg" class="z-[-1] h-full absolute rotate-[180deg] bottom-0 left-[-35%]" alt="" />

    <a href="#" on:click={handleSidebar} class="group pt-[15px] relative flex flex-col items-center gap-[7px]">
      <img
        class={`${
      ...
      <p class={`${sidebarOpen ? 'text-blue' : 'text-grayDark'} text-xs font-medium group-hover:text-blue`}>Menu</p>
    </a>
    ...
    >
      ...
      <p class={`${path === '/casino' ? 'text-blue' : 'text-grayDark'} text-xs font-medium group-hover:text-blue`}>
        Casino
      </p>
    </a>

    <a href="/sports/soccer" class="group pt-[15px] relative flex flex-col items-center gap-[7px]">
      <img
        class={`${
      ...
      <svg class="w-[21px] h-[21px]">
        <use
          class={`${path.includes('/sports') ? 'fill-blue' : 'fill-grayDark'} group-hover:fill-blue`}
          href="/imgs/icons/icons.svg#sports"
        />
      </svg>
      <p
        class={`${path.includes('/sports') ? 'text-blue' : 'text-grayDark'} text-xs font-medium group-hover:text-blue`}
      >
        Sports
      </p>
    </a>

    <a href="#" on:click={chatToggle} class="group pt-[15px] relative flex flex-col items-center gap-[7px]">
      <img
        class={`${
          chatOpen ? '' : 'opacity-0'
        } absolute top-[-9px] min-w-[60px] left-[50%]  translate-x-[-50%] group-hover:opacity-[1] transition-all`}
      ...
      <p class={`${chatOpen ? 'text-blue' : 'text-grayDark'} text-xs font-medium group-hover:text-blue`}>Chat</p>
    </a>

    <a href="#" on:click={toggleBetSlip} class="group pt-[15px] relative flex flex-col items-center gap-[7px]">
      <img
        class={`${
          betSlip ? '' : 'opacity-0'
        } absolute top-[-9px] min-w-[60px] left-[50%]  translate-x-[-50%] group-hover:opacity-[1] transition-all`}
      ...
      <p class={`${betSlip ? 'text-blue' : 'text-grayDark'} text-xs font-medium group-hover:text-blue`}>Bet Slip</p>
    </a>
  </div>
</div>
