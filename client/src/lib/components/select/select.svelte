...
<script>
  ...
  $: selectedValue =
    type == 'crypto' || type == 'fiat'
      ? writable(items.length > 0 ? items[0].value : '')
      : writable($globalStore.userDetail.currentCurrecnySymbol);
  ...
  const handleChange = (item) => {
    if (type == 'crypto' || type == 'fiat') {
      selectedValue = writable(item.value);
    } else {
      let userDetail = $globalStore.userDetail;
      userDetail.currentCurrecnySymbol = item.value;
      userDetail.currentCurrecnyId = item._id;
      userDetail.currentCurrecnyImage = item.image;
      globalStore.toggleItem('userDetail', userDetail);
    }
...
<div class="w-full" on:click={handleFocus}>
  ...
  >
    ...
    <div slot="list" let:filteredItems class="bg-grayLight4 dark:bg-blackLight {listClassName} min-w-[300px]">
      {#if customSearch}
        <div class="w-full p-[10px] z-[999] sticky top-0 bg-grayLight4 dark:bg-blackDark">
      ...
      {#each filteredItems as items}
        ...
        <div
          class={`${
            $selectedValue.value === items.value
              ? 'bg-blue select-selected !opacity-100'
              : 'bg-grayLight2 dark:bg-white5 '
          } cursor-pointer flex items-center gap-[6px] border-b border-grayLight dark:border-blue20 px-[15px] py-[12px]  hover:bg-blue hover:opacity-80`}
