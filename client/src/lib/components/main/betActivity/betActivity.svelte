<script>
  ...
  const updateDynamicBetActivityArr = () => {
...
<div class="w-full flex flex-col items-start gap-[15px] mt-[37px]">
  <div class="w-full flex items-center justify-between">
    <h6 class="font-semibold text-xl gradient-text-white2">Recent Bet Deposits & Withdrawal History</h6>
  </div>

  <div class="relative w-full">
    <ul class="flex flex-wrap items-center gap-[40px] mt-[20px]">
      <li
        on:click={() => handleFilter('latest bets')}
        class={`${
          activeFilter.toLowerCase() === 'latest bets' ? 'active' : ''
        } gradient-text text-xs font-semibold uppercase pb-[14px] cursor-pointer hover-gradient-text transition-all`}
      >
        Latest Bets
      </li>
      <li
        on:click={() => handleFilter('recent Deposits')}
        class={`${
          activeFilter.toLowerCase() === 'recent deposits' ? 'active' : ''
        } gradient-text text-xs font-semibold uppercase pb-[14px] cursor-pointer hover-gradient-text transition-all`}
      >
        Recent Deposits
      </li>
      <li
        on:click={() => handleFilter('recent Withdrawals')}
        class={`${
          activeFilter.toLowerCase() === 'recent withdrawals' ? 'active' : ''
        } gradient-text text-xs font-semibold uppercase pb-[14px] cursor-pointer hover-gradient-text transition-all`}
      >
        Recent Withdrawals
      </li>
    ...
    <div class="w-full flex flex-col px-[28px]">
      {#each dynamicBetActivityArr as item}
        ...
        >
          ...
          <p class="font-semibold text-start text-base text-white">
            {item.player}
          </p>
          ...
          <div class="flex items-center justify-end gap-[6px]">
            {#if item.profitAmount.isProfit}
              <p class="font-semibold text-base text-green">
