<script>
  ...
  import { getCasinoBettingList } from '@apis/casinoBetting';
  ...
  socket.on('bet-history', (data) => {
    ...
    dynamicTableArr = latestBet.slice(0, loadCnt);
  });
        ...
        >
          ...
          <div class="flex items-center gap-[6px]">
            ...
            <img
              ...
              src="https://storage.goodfriendszone.com/uploads/permanent/1703094140698.webp"
              alt="KRW 0"
            />
          </div>
          <p class="text-base font-semibold text-grayDark2">
            {(item.profitAmount / item.betAmount).toFixed(2).toLocaleString()}x
          </p>
          <div class="flex items-center justify-end gap-[6px]">
            <p class={`font-semibold text-base ${item.profitAmount >= item.betAmount ? 'text-green' : 'text-red'}`}>
