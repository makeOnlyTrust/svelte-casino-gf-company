...
<script>
  ...
  const addEmoji = (e) => {
    myMessageText += e.target.textContent;
  };
  ...
  const addGif = (e) => {
    gifMessage = '<div class="gif-chat-item"><img src="' + e.target.src + '"></div>';
    const message = {
      to: roomData,
      from: null,
...
<div
  class="pt-[29px] pb-[18px] bg-gradientWhiteLight dark:bg-gradientBlack w-full"
...
>
  <form on:submit|preventDefault={sendMessage}>
    <div class="relative">
      ...
      <!-- <div
        class="absolute left-[30px] bottom-[50%] translate-y-[-50%] w-[40px] h-[40px]"
        id="emoji-opener-icon"
        on:click={() => (emojiModalOpen = !emojiModalOpen)}
      >
        <img src="/imgs/smiling-eyes.svg" alt="" />
      </div>
      <div
        class="absolute left-[0px] bottom-[50%] translate-y-[-50%] w-[40px] h-[40px]"
