<script>
  ...
  const signInWithTelegram = async (userInfo) => {
    const res = await signupSocial({
...
<p class="mb-[25px] text-sm text-grayDark3 mt-[15px]">
  {$LL.WELCOME_BACK_PLEASE_ENTER_YOUR_DETAILS()}
</p>
<form class="w-full" on:submit={handleSignIn}>
  <div class="flex flex-col w-full gap-[15px]">
    ...
    <a href="#" on:click={openForgotPasswordForm} class="text-end text-msm text-grayDark2">{$LL.FORGOT_PASSWORD()}?</a>
    <button
      type="submit"
  ...
  <div class="flex items-center justify-center mt-[15px] gap-[4px]">
    <p class="text-msm text-grayDark3 font-medium">
      ...
      <a href="#" on:click={openSignUp} class="gradient-text font-bold">{$LL.SIGN_UP()}</a>
    </p>
  </div>
</form>
