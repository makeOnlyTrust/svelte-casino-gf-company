<script>
    ...
    const handleMyProfile = async (event) => {
        if (cropper) {
            ...
            const res = await changeProfileImage({
                customerIdx: $globalStore.userDetail._id,
    ...
    const defaultImgClick = async (e) => {
        const imgSrc =  e.currentTarget.parentNode.querySelector('img').getAttribute('src');
        if(!imgSrc || imgSrc!=='') {
            const res = await changeProfileImage({
                customerIdx: $globalStore.userDetail._id,
                profileImage: imgSrc
            ...
            if (res.success) {
                ...
                toast.error($LL['NICK_CHANGE_FAILED']());
        }
    }
</script>
        ...
        <div class="relative z-[9] w-full flex flex-col gap-[16px] mt-[24px]">
            ...
            <div class="flex items-center flex-wrap gap-[15px]">
                <label class="cursor-pointer">
                    <input type="radio" name="defaultImg" class="peer sr-only" on:click={defaultImgClick}/>
                    <div
                ...
                <label class="cursor-pointer">
                    <input type="radio" name="defaultImg" class="sr-only peer" on:click={defaultImgClick}/>
                    <div
                            class="w-[78px] h-[78px] border-[2px] rounded-full border-transparent peer-checked:border-blue flex items-center justify-center transitin-al"
                    >
                        <div class="w-[69px] h-[69px] rounded-full">
