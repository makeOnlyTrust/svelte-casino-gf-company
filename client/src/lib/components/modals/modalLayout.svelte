<script>
  ...
  onMount(() => {
    document.addEventListener('click', (e) => {
      if (
        e.target.closest('#modalLayout') &&
        !e.target.closest('#modalInner') &&
        !e.target.closest('#closeForm') &&
        !e.target.closest('.radialImg')
      ) {
        dispathEvent('closeProfileModal');
      }
    });
...
<div
  id="modalLayout"
  class="fixed top-[0px] left-0 w-full h-screen dark:bg-black21 backdrop-blur-[5px] !z-[99999999] md:z-[9999] overflow-auto"
>
        ...
        <div class="h-full flex flex-col w-full">
          <div class="w-full flex justify-between px-[26px]">
            <div class="flex items-center gap-[15px]">
              ...
              {/if}
              <p class="text-xl font-semibold gradient-text-white2 main">
                {title}
              </p>
            </div>
            <div
