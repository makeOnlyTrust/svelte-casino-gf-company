<script>
  ...
  import { getMinigameLists } from '@apis/minigame';
  ...
  const load = async () => {
    casinoList = [];
    slotsLoaded = false;
    const _type = data.category === 'type' ? data.type : '';
    const _vendor = data.category === 'vendors' ? data.type : '';
    const userId = $globalStore.userDetail ? $globalStore.userDetail._id : '';

    const res = await getMinigameLists(userId, _type, _vendor);
...
<main class="w-full py-[27px] pl-[15px] pr-[15px] md:pl-[30px] md:pr-[30px]" id="main">
  <div class="container-main">
    <div class="grid grid-cols-[auto] gap-[4px] rounded-[7px] overflow-hidden h-[50px] hidden">
      <div
    ...
    {#if casinoList.length > 0}
      <div class="grid grid-system-slots w-full gap-[20px] mt-[20px]">
        {#each casinoList as item}
          ...
          <MinigameItem item={item} load={false} type={item.category} />
        {/each}
